...
// ── Update Leave Dates (Admin-only) ──────────────────────────────
const updateLeaveDates = async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res.status(403).json({ message: "Access denied" });
    }
    ...
    leave.from = new Date(from);
    leave.to = new Date(to);
    const diffTime = Math.abs(leave.to - leave.from);
    leave.days = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;
    await leave.save();

    res.json({ message: "Leave dates updated", leave: shapeLeave(leave) });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
