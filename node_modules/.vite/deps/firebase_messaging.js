import { A as isIndexedDBAvailable, C as deleteDB, D as FirebaseError, E as ErrorFactory, O as areCookiesEnabled, T as Component, c as _getProvider, f as _registerComponent, g as getApp, j as validateIndexedDBOpenable, k as getModularInstance, w as openDB, x as registerVersion } from "./index.esm-2Adt1GqY.js";
...
function getDbPromise$1() {
	if (!dbPromise$1) dbPromise$1 = openDB(DATABASE_NAME$1, DATABASE_VERSION$1, { upgrade: (db, oldVersion) => {
		switch (oldVersion) {
...
async function set(appConfig, value) {
	const key = getKey$1(appConfig);
	const tx = (await getDbPromise$1()).transaction(OBJECT_STORE_NAME, "readwrite");
...
function extractAppConfig$1(app) {
	...
	for (const keyName of [
		...
		"apiKey",
		"appId"
	]) if (!app.options[keyName]) throw getMissingValueError$1(keyName);
	return {
		appName: app.name,
...
var publicFactory = (container) => {
	...
	return {
		app,
		appConfig: extractAppConfig$1(app),
...
async function dbRemove(firebaseDependencies) {
	const key = getKey(firebaseDependencies);
	const tx = (await getDbPromise()).transaction(TOKEN_OBJECT_STORE_NAME, "readwrite");
...
async function requestDeleteRegistration(firebaseDependencies, fid) {
	...
	try {
		...
		throw ERROR_FACTORY.create("fid-unregister-failed", { errorInfo: err?.toString() });
	}
	if (response.ok) return;
	try {
		throw (await response.json()).error?.message ?? response.statusText;
	} catch (err) {
		throw ERROR_FACTORY.create("fid-unregister-failed", { errorInfo: typeof err === "string" && err || response.statusText || err?.toString() });
	}
}
/**
...
async function revokeFidRegistrationIfStored(messaging) {
	const fid = (await dbGetFidRegistration(messaging.firebaseDependencies).catch(() => void 0))?.fid;
	if (fid) await requestDeleteRegistration(messaging.firebaseDependencies, fid);
	await removeFidRegistrationBestEffort(messaging.firebaseDependencies);
	if (fid) notifyOnUnregistered(messaging, fid);
}
/**
...
async function revokeRegistrationInternal(messaging) {
	const tokenDetails = await dbGet(messaging.firebaseDependencies);
	if (tokenDetails) await revokeLegacyFcmTokenAndClearCaches(messaging, tokenDetails);
	else await revokeFidRegistrationIfStored(messaging);
...
function onRegistered$1(messaging, nextOrObserver) {
	...
	return () => {
		if (messaging.onRegisteredHandler === nextOrObserver) messaging.onRegisteredHandler = null;
	};
}
/**
* @license
...
function onUnregistered$1(messaging, nextOrObserver) {
	...
	return () => {
		if (messaging.onUnregisteredHandler === nextOrObserver) messaging.onUnregisteredHandler = null;
	};
}
/**
* @license
...
async function unregister$1(messaging) {
	if (!navigator) throw ERROR_FACTORY.create("only-available-in-window");
	const fid = (await dbGetFidRegistration(messaging.firebaseDependencies).catch(() => void 0))?.fid ?? await messaging.firebaseDependencies.installations.getId();
